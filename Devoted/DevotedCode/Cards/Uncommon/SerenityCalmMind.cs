...
public class SerenityCalmMind() : DevotedCard(3, CardType.Power, CardRarity.Uncommon, TargetType.Self)
{
    
    protected override HashSet<CardTag> CanonicalTags => [MyCustomEnums.SerenityCard];    
    
    protected override IEnumerable<DynamicVar> CanonicalVars => [new PowerVar<SerenityCardCreatePower>(1M)];
    ...
    protected override async Task OnPlay(MegaCrit.Sts2.Core.GameActions.Multiplayer.PlayerChoiceContext choiceContext,
        CardPlay play)
    {
        SerenityCalmMind cardSource = this;
        await CreatureCmd.TriggerAnim(cardSource.Owner.Creature, "Cast", cardSource.Owner.Character.CastAnimDelay);
        if (cardSource.IsUpgraded)
        {
            SerenityUpgradedCardCreatePower demonFormCreatePower = (await PowerCmd.Apply<SerenityUpgradedCardCreatePower>(choiceContext,
        ...
        {
            SerenityCardCreatePower demonFormCreatePower = (await PowerCmd.Apply<SerenityCardCreatePower>(choiceContext,
                cardSource.Owner.Creature, cardSource.DynamicVars["SerenityCardCreatePower"].BaseValue,
                cardSource.Owner.Creature, (CardModel)cardSource))!;
        }
    }
